...
class Course extends Model
{
    ...
    {
        if(Config('admin.courses.sections'))
        {
    ...
    {
        $courses = static::where('status', 1)->get();
        $output = $this->parse_hierarchical_list($courses);
        return $output;
    }
        ...
        if($array)
            foreach ($array as $key => $item) {
                if($item->parent_id == $currentParent){
                    ...
                    $html .= '<div class="checkbox">'.$indent.'<input type="checkbox" name="services_links[]" class="services_links" value="services_link-'.$item->id.'" data-id="'.$item->id.'" data-name="'.$item->name.'" id="courses-'.$item->id.'"> <label for="courses-'.$item->id.'">'.$item->name.'</label></div>';
